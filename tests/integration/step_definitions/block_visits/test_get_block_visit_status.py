...
@when(
    parsers.parse("I request a block visit {block_visit_id} status"), target_fixture="response"
)
...
def block_visit(block_visit_id: int, status_value: str, client: TestClient) -> None:
    response = client.get(BLOCK_VISIT_URL + "/" + str(block_visit_id) + "/status", params={
        "proposal_code": "2016-1-COM-001"
    })
